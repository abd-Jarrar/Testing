...
namespace Testing
{
    public class SalarySlipProcessor
    {
        ...
        public decimal CalculateDependancyAllowance(Employee employee)
        {
            ...
            if (employee.TotalDependencies < 0)
                throw new ArgumentOutOfRangeException($"invalid {nameof(employee.TotalDependencies)}");


            if (employee.TotalDependencies > Constants.MaxDependantsFactor)
            ...
            {
                return employee.TotalDependencies * Constants.DependancyAllowancePerChildAmount;
            }
        }
